...
def compute_autoregressive_trend_loss(
    *,
    ...
    physics_grid: object | None = None,
    physics_cfg: object | None = None,
    ...
    # D5 baseline: pure-DL single-trend rollout. Physics-related paths are disabled.
    if bool(online_physics_coupling):
        raise ValueError("D5 is pure-DL. online_physics_coupling must be False.")
    if physics_trend_seq is not None:
        raise ValueError("D5 is pure-DL. physics_trend_seq must be None.")
    if physics_grid is not None or physics_cfg is not None:
        raise ValueError("D5 is pure-DL. physics_grid/physics_cfg must be None.")
    if diffusion_task_model is not None:
        raise ValueError("D5 is pure-DL. diffusion_task_model must be None.")
    if bool(learn_diffusion_coeff):
        raise ValueError("D5 is pure-DL. learn_diffusion_coeff must be False.")
    ...
    for s in range(steps):
        ...
        trend_phys = torch.zeros_like(trend_dl)
        ...
        loss_s = float(cfg.aod_task_weight) * loss_state_s + float(cfg.sparsity_weight) * sparsity_s
