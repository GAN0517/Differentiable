...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--physics-adv-constraint-weight",
        type=float,
        default=float(TRAIN_SETTINGS.get("physics_adv_constraint_weight", LossConfig().physics_adv_weight)),
        help="Physics constraint weight for advection module.",
    )
    parser.add_argument(
        "--physics-div-constraint-weight",
        type=float,
        default=float(TRAIN_SETTINGS.get("physics_div_constraint_weight", LossConfig().physics_div_weight)),
        help="Physics constraint weight for divergence/compression module.",
    )
    parser.add_argument(
        "--physics-diff-constraint-weight",
        type=float,
        default=float(TRAIN_SETTINGS.get("physics_diff_constraint_weight", LossConfig().physics_diff_weight)),
        help="Physics constraint weight for diffusion module.",
    )
    parser.add_argument(
        "--physics-forward-mode",
        choices=("pure_dl", "coupled"),
        default=str(TRAIN_SETTINGS.get("physics_forward_mode", "pure_dl")),
        help=(
            "Physics-aware rollout mode in trend_sum: "
            "'pure_dl' keeps A_{t+1}=A_t+dt*trend_dl; "
            "'coupled' uses A_{t+1}=A_t+dt*(trend_dl+trend_phys)."
        ),
    )
    parser.add_argument(
        "--physics-loss-normalize",
        action=argparse.BooleanOptionalAction,
        default=bool(TRAIN_SETTINGS.get("physics_loss_normalize", True)),
        help="Normalize physics residual loss by trend energy scale each rollout step.",
    )
    parser.add_argument(
        "--sparsity-weight",
        type=float,
    ...
    if float(args.physics_adv_constraint_weight) < 0.0:
        raise ValueError(f"--physics-adv-constraint-weight must be >= 0, got {args.physics_adv_constraint_weight}")
    if float(args.physics_div_constraint_weight) < 0.0:
        raise ValueError(f"--physics-div-constraint-weight must be >= 0, got {args.physics_div_constraint_weight}")
    if float(args.physics_diff_constraint_weight) < 0.0:
        raise ValueError(f"--physics-diff-constraint-weight must be >= 0, got {args.physics_diff_constraint_weight}")
    return args
    ...
    for batch in loader:
        ...
        if prediction_mode == "trend_sum":
            ...
            loss, _, parts = compute_autoregressive_trend_loss(
                ...
                physics_loss_normalize=bool(physics_loss_normalize),
            )
        else:
...
def _run_single(args: argparse.Namespace) -> int:
    ...
    if args.prediction_mode == "trend_sum":
        ...
        logging.info(
            "D5PL mode: trend rollout + physics residual constraint | forward_mode=%s | physics_loss_normalize=%s",
            str(args.physics_forward_mode),
            bool(args.physics_loss_normalize),
        )

    use_physics_terms = (
        args.prediction_mode == "trend_sum"
        and (
            str(args.physics_forward_mode) == "coupled"
            or
            float(args.physics_adv_constraint_weight) > 0.0
            or float(args.physics_div_constraint_weight) > 0.0
            or float(args.physics_diff_constraint_weight) > 0.0
        )
    )
    if use_physics_terms:
        if lat_values is None or lon_values is None:
            raise RuntimeError("Physics-enabled rollout/loss requires latitude/longitude coordinates.")
        if u10_feature_index is None or v10_feature_index is None:
            raise RuntimeError("Physics-enabled rollout/loss requires U10/V10 features in input.")
    else:
        logging.info("Physics terms disabled (pure DL forward and zero physics constraint weights).")
    ...
    if use_physics_terms:
        ...
        logging.info(
            "Physics constraints enabled | adv_w=%.3f div_w=%.3f diff_w=%.3f | advection=%s diffusion=%s k_const=%.3e cos_floor=%.3f | forward_mode=%s | normalize=%s",
            float(args.physics_adv_constraint_weight),
            float(args.physics_div_constraint_weight),
            float(args.physics_diff_constraint_weight),
    ...
    loss_cfg = LossConfig(
        ...
        rh_task_weight=0.0,
        physics_adv_weight=float(args.physics_adv_constraint_weight),
        physics_div_weight=float(args.physics_div_constraint_weight),
        physics_diff_weight=float(args.physics_diff_constraint_weight),
    )
    if args.prediction_mode == "trend_sum":
        logging.info(
    ...
    for epoch in range(1, args.epochs + 1):
        train_total, train_mse, train_extra = run_epoch(
            ...
            physics_forward_mode=str(args.physics_forward_mode),
            physics_loss_normalize=bool(args.physics_loss_normalize),
        )
    ...
    torch.save(
        {
            ...
            "physics_adv_constraint_weight": float(args.physics_adv_constraint_weight),
            "physics_div_constraint_weight": float(args.physics_div_constraint_weight),
            "physics_diff_constraint_weight": float(args.physics_diff_constraint_weight),
            "physics_forward_mode": str(args.physics_forward_mode),
            "physics_loss_normalize": bool(args.physics_loss_normalize),
            "max_points_per_forward": args.max_points_per_forward,
            "lr_src": args.lr_src,
