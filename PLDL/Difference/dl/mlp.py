...
class ProcessDecomposedMLP(nn.Module):
    ...
    def __init__(self, cfg: ProcessDecomposedMLPConfig):
        ...
        # Radiation trunk input: [AOD_prev, WS, CERES, SZA, T2M, AODADV, AODCON]
        self.rad_trunk = _build_trunk(
            input_dim=7,
            hidden_dims=cfg.hidden_dims,
            dropout=cfg.dropout,
        )
        rad_dim = int(cfg.hidden_dims[-1]) if len(cfg.hidden_dims) > 0 else 7
        self.head_trend_rad = nn.Linear(rad_dim, 1)
        self.head_t2m_next = nn.Linear(rad_dim, 1)
        self.head_ceres_next = nn.Linear(rad_dim, 1)

        # Deposition trunk input: [AOD_prev, WS, BLH, T2M, TP, CLW, RH, SP, AODADV, AODCON]
        self.dep_trunk = _build_trunk(
            input_dim=10,
            hidden_dims=cfg.hidden_dims,
            dropout=cfg.dropout,
        )
        dep_dim = int(cfg.hidden_dims[-1]) if len(cfg.hidden_dims) > 0 else 10
    ...
    def forward_components(
        ...
        *,
        ...
        rad_in = torch.cat([aod_prev, ws, ceres, sza, t2m, aod_adv_flat, aod_con_flat], dim=-1)
        dep_in = torch.cat([aod_prev, ws, blh, t2m, tp, clw, rh, sp, aod_adv_flat, aod_con_flat], dim=-1)
